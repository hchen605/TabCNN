...
def pitch_precision(pred, gt):
    pitch_pred = np.array(list(map(tab2pitch,pred)))#HH
    pitch_gt = np.array(list(map(tab2pitch,gt)))#HH
...
def tab_precision(pred, gt):
    ...
    tab_pred = np.array(list(map(tab2bin,pred)))#HH
    tab_gt = np.array(list(map(tab2bin,gt)))#HH
